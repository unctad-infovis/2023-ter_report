...
function LineChart({
  allow_decimals, data, idx, line_width, note, show_first_label, source, subtitle, title, xlabel, ymax, ymin, ytickinterval
}) {
    ...
    Highcharts.chart(`chartIdx${idx}`, {
      ...
      tooltip: {
        ...
        formatter() {
          ...
          const rows = [];
          rows.push(values.map(point => `<div><span class="tooltip_label">${(point[0]) ? `${point[0]}: ` : ''}</span> <span class="tooltip_value">${roundNr(point[1], 1).toFixed(1).toLocaleString('en-US')}%</span></div>`).join(''));
          // eslint-disable-next-line react/no-this-in-sfc
